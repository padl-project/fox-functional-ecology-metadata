...
`chlorurus_activity <- read_csv(glue(
"{dir_gdrive}/DATA_RAW/Peter Carlson/Chlorurus_Activity_data.csv")) %>% 
clean_names()`

- To add multiple files from the drive into a tibble 

`multiple_files <- tibble(list.files(glue(
"{dir_gdrive}/Data/Peter Carlson/"), pattern = ".csv"))`
...
"{dir_gdrive}/Data/Mike Fox/fox-functional-ecology-DRYAD"), pattern = ".csv")) %>% 
  rename(file_name = 1)

bulk13c <- read_csv(glue(
"{dir_gdrive}/Data/Mike Fox/fox-functional-ecology-DRYAD/", list_all_csv$file_name[1]))
